...
- [Конфигурация](#конфигурация)
  ...
  - [Конфигурация генерации данных](#конфигурация-генерации-данных)
    - [Конфигурация типа данных (поле `models[*].columns[*].type_params` или `models[*].columns[*].ranges.type_params`)](#конфигурация-типа-данных-поле-modelscolumnstype_params-или-modelscolumnsrangestype_params)
    - [Конфигурация выходного формата (поле `params`)](#конфигурация-выходного-формата-поле-params)
...
- `output`: Конфигурация вывода сгенерированных данных, описывается структурой `OutputConfig`.

Структура `OutputConfig` описывает конфигурацию вывода сгенерированных данных:
...
- `params`: Параметры для выбранного типа выходного формата. Структура этого поля описана [здесь](#конфигурация-выходного-формата-поле-params). 
- `checkpoint_interval`: Параметр, отвечающий за то, как часто будут обновляться файлы, содержащие текущий прогресс. По умолчанию 5 секунд.
...
- `type_params`: Параметры для выбранного типа данных.
...
Структура поля `models[*].columns[*].parquet_params`:
...
#### Конфигурация типа данных (поле `models[*].columns[*].type_params` или `models[*].columns[*].ranges.type_params`)

Структура поля для типа данных `integer`:
...
Структура поля для типа данных `float`:
...
Структура поля для типа данных `string`:
...
Структура поля для типа данных `datetime`:
...
#### Конфигурация выходного формата (поле `params`)

Структура поля для формата `csv`:
...
Структура поля для формата `parquet`:
...
Структура поля для формата `http`:
...
Доступно 2 поля для использования в шаблоне:
...
Структура поля для формата `tcs`:
...
> **Важно**: для корректного продолжения генерации нельзя менять конфигурацию генерации и удалять папку, в которую записывались данные.
